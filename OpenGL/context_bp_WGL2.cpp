...
#define VIEWSTYLE   WS_VISIBLE|WS_POPUP|WS_MAXIMIZE
        ...
        HWND OpenGLWindowCreate(HINSTANCE hInstance, std::wstring title, aabb2 bb)
        {
            ...
            if (wglChoosePixelFormatARB && wglCreateContextAttribsARB) {
                ...
                if (!hWnd) {
                    if (!wglMakeCurrent(NULL, NULL)) { deb::debss << "error making context current"; deb::msberr(); std::terminate(); }
                ...
                if (!DescribePixelFormat(hDC, iPF, sizeof(pfd), &pfd)) { deb::debss << "error describe"; deb::msberr(); std::terminate(); }
                ...
                if (!wglMakeCurrent(hDC, hRC)) { deb::debss << "error making context current"; deb::msberr(); std::terminate(); }

                deb::msbinfo("got hRC and made current");
                if (com::getLastError()) std::terminate();

                //deb::debss << "windows " << hTempWnd << " " << hWnd; deb::msbinfo();

                /* now that we've created the proper window, DC and RC
                 * we can delete the temporaries */
                if (!wglMakeCurrent(NULL, NULL)) { deb::debss << "error making context current"; deb::msberr(); std::terminate(); }
                deb::msbinfo("wglMakeCurrent null1");
                if(!wglDeleteContext(hTempRC)) { deb::debss << "error wglDeleteContext"; deb::msberr(); std::terminate(); }
                deb::msbinfo("wglMakeCurrent null2");
                if(!ReleaseDC(hTempWnd, hTempDC)) { deb::debss << "error release dc"; deb::msberr(); std::terminate(); }
                deb::msbinfo("wglMakeCurrent null3");
                if (!DestroyWindow(hTempWnd)) { deb::debss << "error destroy window"; deb::msberr(); std::terminate(); }
                deb::msbinfo("wglMakeCurrent null4");
		...
		HWND createWindow(HINSTANCE hInstance, std::wstring title, aabb2 bb) {
            ...
            if (!wglMakeCurrent(viewwnd::hDC, viewwnd::hRC)) { deb::debss << "error making context current"; deb::msberr(); std::terminate(); }
