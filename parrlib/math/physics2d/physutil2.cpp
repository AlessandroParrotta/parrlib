...
namespace prb {
	namespace physutil2 {
		std::shared_ptr<Collision2D> checkCollision(std::vector<vec2> const& ps1, mat3 const& tr1, std::vector<vec2> const& axes1, std::vector<vec2> const& ps2, mat3 const& tr2, std::vector<vec2> const& axes2) {
			float overlap = FLT_MAX;
			Vector2f smallestAxis = FLT_MAX;

			for (int i = 0; i < axes1.size(); i++) {
				Projection2D p1(ps2, tr2, axes1[i]);
				Projection2D p2(ps1, tr1, axes1[i]);
			...
			for (int i = 0; i < axes2.size(); i++) {
				Projection2D p1 = Projection2D(ps2, tr2, axes2[i]);
				Projection2D p2 = Projection2D(ps1, tr1, axes2[i]);
		...
		std::shared_ptr<Collision2D> checkCollision(aabb2 const& bb1, aabb2 const& bb2) {
			...
			proj2 p0(bb1.minx(), bb1.maxx());
			proj2 p1(bb2.minx(), bb2.maxx());

			proj2 p2(bb1.miny(), bb1.maxy());
			proj2 p3(bb2.miny(), bb2.maxy());
			...
			return std::make_shared<coll2>(smallestax, minov);
		}
		...
		std::shared_ptr<Collision2D> checkCollision(vec2 const& p, aabb2 const& bb) {
			...
			return std::make_shared<Collision2D>(cmp ? vec2x(1.f) : vec2y(1.f), cmp ? minovx : minovy);
		}
	}
}
