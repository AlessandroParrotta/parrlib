...
namespace utils2 {
	...
	std::vector<vec2> transform(std::vector<vec2> const& poly, mat3 const& tr) {
		std::vector<vec2> res; res.reserve(poly.size());
		for (int i = 0; i < poly.size(); i++) res.push_back(tr * poly[i]);
		return res;
	}

	std::wostream& operator<<(std::wostream& wo, Line const& ln) { wo << "(" << ln.p1 << ", " << ln.p2 << ")"; return wo; }
	...
	bool lineIntersOpti(Line const& l1, Line const& l2) {
		...
		Projection2D p0(l2.p1, l2.p2, ax0);
	...
	bool intersLineQuad(Line const& l, aabb2 const& quad) {
		...
		Projection2D p0(quad, ax0);
		if (selfproj < p0.min || selfproj > p0.max) return false;
		
		vec2 ax1 = v2ax::upv2;
		Projection2D p1(quad, ax1);
		Projection2D p2(l.p1, l.p2, ax1);
	...
	Line getPolyEdge(int i, std::vector<Vector2f> const& poly, int dir) {
		return Line(poly[i], poly[outl::pmod(i + dir, poly.size())]);
	}
	...
	vec2 getCenter(std::vector<vec2> const& poly) {
		vec2 res = 0.f;
		for (int i = 0; i < poly.size(); i++) res += poly[i];
		res /= (float)poly.size();
		return res;
		//return aabb2(poly).center();
	}
	...
	std::vector<LinePolyInters> lineIntersPoly(Line const& l, std::vector<Vector2f> const& poly) { return lineIntersPoly(l, poly, Vector2f()); }
	...
	bool lineIntersPolyCheck(Line const& l, std::vector<Vector2f> const& poly) { return lineIntersPolyCheck(l, poly, Vector2f()); }
	...
	std::vector<PolyPolyInters> polyIntersPoly(std::vector<Vector2f> const& poly1, std::vector<Vector2f> const& poly2) { return polyIntersPoly(poly1, poly2, Vector2f()); }
	...
	bool polyIntersPolyCheck(std::vector<Vector2f> const& poly1, std::vector<Vector2f> const& poly2) { return polyIntersPolyCheck(poly1, poly2, Vector2f()); }
	...
	Vector2f correct(std::vector<Vector2f> &mesh, int id, int dir) {
		...
		int id1 = cw ? (id + 1) % mesh.size() : outl::pmod(id - 1, mesh.size()),
			id2 = cw ? outl::pmod(id - 1, mesh.size()) : (id + 1) % mesh.size();
	...
	Vector2f correctCW(std::vector<Vector2f>& mesh, int id, int dir) {
		int id1 = (id + 1) % mesh.size(),
			id2 = outl::pmod(id - 1, mesh.size());
		...
		return mesh[id] + perp * (tolerance*2.f * (float)dir);
	}
	...
	std::vector<BrokenPoly> breakPoly(std::vector<Vector2f> destructor, std::vector<Vector2f> poly, int startMeshSize, int level) {
		...
		if (outInters[0].id != polyInIter) {
			...
			while (secondAssembleIter != (polyInIter+1)%poly.size()) {
				secondAssembleIter--;
				secondAssembleIter = outl::pmod(secondAssembleIter, poly.size());
