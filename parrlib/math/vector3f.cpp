...
namespace prb {
	...
	Vector3f Vector3f::perp(Vector3f const& dir) { return cross(dir).normalized(); }
	Vector3f Vector3f::perp() { return cross(v3ax::upv3).normalized(); }
	...
	namespace v3u {
		vec3 x(float x) { return { x, 0.f, 0.f }; }
		vec3 y(float y) { return { 0.f, y, 0.f }; }
		vec3 z(float z) { return { 0.f, 0.f, z }; }

		vec3 get(std::stringstream& ss) { return { stru::getf(ss), stru::getf(ss), stru::getf(ss), }; }
		vec3 get(std::ifstream& f)		{ return { stru::getf(f), stru::getf(f), stru::getf(f) }; }
	}
