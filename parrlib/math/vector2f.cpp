...
namespace prb {
	...
	Vector2f Vector2f::operator+ (float other) const { return Vector2f(x + other, y + other); }
	Vector2f Vector2f::operator- (float other) const { return Vector2f(x - other, y - other); }
	Vector2f Vector2f::operator* (float other) const { return Vector2f(x * other, y * other); }
	Vector2f Vector2f::operator/ (float other) const { return Vector2f(x / other, y / other); }

	Vector2f Vector2f::operator+= (float other) { x += other; y += other;  return *this; }
	Vector2f Vector2f::operator-= (float other) { x -= other; y -= other;  return *this; }
	Vector2f Vector2f::operator*= (float other) { x *= other; y *= other;  return *this; }
	Vector2f Vector2f::operator/= (float other) { x /= other; y /= other;  return *this; }
	...
	float Vector2f::dot(Vector2f const& other) const {
		return x * other.x + y * other.y;
	}

	float Vector2f::det(Vector2f const& v) const { return x * v.y - y * v.x; }
	...
	float Vector2f::cross(Vector2f const& v) const {
		return x * v.y - y * v.x;
	}
	...
	Vector2f Vector2f::lerp(Vector2f const& v, float t) const { return (v - *this) * t; }
	...
	Vector2f Vector2f::modded(Vector2f v) const { return Vector2f(fmodf(x, v.x), fmodf(y, v.y)); }
	...
	bool Vector2f::inRange(Vector2f min, Vector2f max) const {
	...
	Vector2f Vector2f::xf(float f) const { return { x, f }; }
	Vector2f Vector2f::fy(float f) const { return { f, y }; }

	Vector2f Vector2f::mx(float f) const { return { x+f, y }; }
	Vector2f Vector2f::my(float f) const { return { x, y+f }; }
	...
	namespace v2u {
		...
		vec2 get(std::stringstream& ss) { return { stru::getf(ss), stru::getf(ss) }; }
		vec2 get(std::ifstream& f)		{ return { stru::getf(f), stru::getf(f) }; }
	}
}
