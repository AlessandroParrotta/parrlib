...
namespace prb{
	namespace imui {
		...
		vec4 getTextOutlineColor() { /*return getTxr().outlineColor();*/ return getTxr().color(); }
		...
		mat3 getNDCTr() { return pmat3::scale(1.f / cst::res() * 2.f); }

		void drawstr(std::wstring const& str, std::wstring const& modstr, vec2 const& off, aabb2 const& bb, mat3 const& tr) {
			glb::getTxr(glbid).boundReplacer(modstr); glb::getTxr(glbid).drawOffset(off); glb::getTxr(glbid).drawTextBounded(str, tr * getNDCTr(), space * bb);
		}
		void drawstr(std::wstring const& str, vec2 const& off, aabb2 const& bb, mat3 const& tr) {
			glb::getTxr(glbid).boundReplacer(L"..."); glb::getTxr(glbid).drawOffset(off); glb::getTxr(glbid).drawTextBounded(str, tr * getNDCTr(), space * bb);
		}

		void drawstr(std::wstring const& str, vec2 const& off, mat3 const& tr) {
			glb::getTxr(glbid).boundReplacer(L"..."); glb::getTxr(glbid).drawOffset(off); glb::getTxr(glbid).drawText(str, tr * getNDCTr()); //TODO: offset taken into account in drawText
		}

		TextRenderer::bndtx strbbw(std::wstring const& str, vec2 const& off, aabb2 const& bb, mat3 const& tr){
			//return glb::getTxr(glbid).getWStringBoundedpx(str, space * bb, off, L"", tr);
			glb::getTxr(glbid).boundReplacer(L""); glb::getTxr(glbid).drawOffset(off); return glb::getTxr(glbid).batchTextBounded(str, tr * getNDCTr(), space * bb);
		}

		TextRenderer::bndtx strbbw(std::wstring const& str, aabb2 const& bb, vec2 const& off, std::wstring const& modstr, mat3 const& tr) {
			glb::getTxr(glbid).boundReplacer(modstr); glb::getTxr(glbid).drawOffset(off); return glb::getTxr(glbid).batchTextBounded(str, tr * getNDCTr(), space * bb);
		}

		aabb2 strbb(std::wstring const& str, vec2 const& off, aabb2 const& bb, mat3 const& tr) {
			return strbbw(str, off, bb, tr).bb;
		}
		...
		void txreptext(textfieldo& tf, int o1, int o2, std::wstring const& str) {
			//deb::pr("before: ",*tf.text,"\n");
			remtext(tf, o1, o2);
			apptext(tf, str, o1);
			//deb::pr("after: ", *tf.text, "\n");
		}
		...
		int tfgetsel(textfieldo& tf, vec2 const& v, aabb2 const& stbb, vec2 const& off, aabb2 const& ta, mat3 const& tr) {
			...
			TextRenderer::bndtx shadowbb = strbbw(tf.vistext, mbb, off, L"", tr);
		...
		int textField(std::string const& identifier, std::wstring const& hintText, std::wstring* text) {
			...
			deb::prt("'",*tf.text,"'", " '", tf.vistext,"' ",tf.off, " ", tf.sel, " ", tf.selmax,"\n");
			...
			if (tf.active) {
				...
				TextRenderer::bndtx bwstr = strbbw(tf.vistext, off, ta, tr);
				aabb2 stbb = bwstr.bb;

				util::setColor(vc4::red);
				util::drawQuad(stbb, space.inverted());
				...
				if (input::getKey(VK_BACK) && txcontrol(tf, speed)) {
				...
				if (input::getKey(VK_LEFT) && txcontrol(tf, speed)) {
					if (input::getKey(VK_LCONTROL)) {
				...
				if (input::getKey(VK_RIGHT) && txcontrol(tf, speed)) {
					if (input::getKey(VK_LCONTROL)) {
						tf.sel = outl::clamp(tf.sel, 0, tf.text->length() - 1);
				...
				if (input::getKey(VK_LCONTROL) && txcontrol(tf, speed)) {
					if (input::getKey('V')) {
				...
				if (input::getKeyDown('X')) tfmoveoff(tf, 1, off, ta, tr);
				if (input::getKeyDown('Z')) tfmoveoff(tf, -1, off, ta, tr);
				...
				if (input::getKeyDown(VK_RETURN)) return 3;
