...
namespace imui {
	...
	void drawstr(std::wstring const& str, std::wstring const& modstr, vec2 const& off, aabb2 const& bb, mat3 const& tr) {
		...
		glb::getTxr(glbid).drawText(str, off);
	}
	...
	aabb2 strbbw(std::wstring const& str, vec2 const& off, aabb2 const& bb, mat3 const& tr){
		//return glb::getTxr(glbid).getWStringBoundedpx(str, space * bb, off, L"", tr);
		return glb::getTxr(glbid).getTextBoundingBox(str, off);
	}

	//TextRenderer::bwstring strbbw(std::wstring const& str, aabb2 const& bb, vec2 const& off, std::wstring const& modstr, mat3 const& tr) {
	aabb2 strbbw(std::wstring const& str, aabb2 const& bb, vec2 const& off, std::wstring const& modstr, mat3 const& tr) {
	...
	int tfgetsel(textfieldo& tf, vec2 const& v, aabb2 const& stbb, vec2 const& off, aabb2 const& ta, mat3 const& tr) {
		...
		aabb2 shadowbb = strbbw(tf.vistext, mbb, off, L"", tr);
	...
	void tfmoveoff(textfieldo& tf, int delta, vec2 const& off, aabb2 const& ta, mat3 const& tr) {
		...
		tf.selvis = tf.off + tf.text->substr(tf.off, tf.text->length() - tf.off).length();
		tfadjustsels(tf);
	}
	...
	int textField(std::string const& identifier, std::wstring const& hintText, std::wstring* text) {
		...
		if (tf.active) {
			...
			aabb2 bwstr = strbbw(tf.vistext, off, ta, tr);
			...
			tf.selvis = tf.off + tf.vistext.length(); //max character visible on screen
			...
			if (input::getKey(VK_CONTROL) && txcontrol(tf, speed)) {
				if (input::getKey('V')) {
			...
			if (input::getKey(VK_DELETE)	||
				...
				(input::getKey(VK_CONTROL) && input::getKey('V')) ||
