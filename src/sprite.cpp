...
Sprite::Sprite(const char* tex) : Sprite(Texture(tex)) {
	...
	if (!f.good()) { std::cout << "could not find sprite '" << tex << "'\n"; return; }
...
void Sprite::drawTiled(AnimationPlayer const& ap, vec2 const& size, mat3 const& transform) const {
	...
	util::drawSubTex(tex.texID, coords[4], offs[4] / tex.size, transform);

	if (diff.fmin() != 0.f || diff.fmax() != 0.f) {
		for (int i = 0; i < 9; i++) {
			if (i != 4) util::drawSubTex(tex.texID, coords[i], offs[i] / tex.size, transform);
		}
	}
